...
Quat normalize(Quat value) {
  const float l = std::sqrt(value[0] * value[0] + value[1] * value[1] +
...
Float3 rotate(const Quat &q, const Float3 &value) {
  ...
  return add(
      add(mul(u, 2.0F * dot(u, value)), mul(value, q[3] * q[3] - dot(u, u))),
...
Quat axisAngle(const Float3 &axis, float angle) {
  ...
  return normalize(
      {axis[0] * sine, axis[1] * sine, axis[2] * sine, std::cos(half)});
}
...
float bezier(float x, std::uint8_t x1, std::uint8_t y1, std::uint8_t x2,
             std::uint8_t y2) {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    const float curveX =
        3.0F * inv * inv * t * ax + 3.0F * inv * t * t * bx + t * t * t;
    if (curveX < x)
      low = t;
    else
      high = t;
...
BezierLut makeBezierLut(std::uint32_t id) {
  ...
  for (std::size_t i = 0; i < result.size(); ++i) {
    result[i] =
        bezier(static_cast<float>(i) / static_cast<float>(result.size() - 1U),
               x1, y1, x2, y2);
  }
  return result;
}
...
float sampleBezier(const BezierLut &lut, float x) {
  const float scaled =
      std::clamp(x, 0.0F, 1.0F) * static_cast<float>(lut.size() - 1U);
...
template <typename Key> auto nextKey(KeyTrack<Key> &track, float frame) {
  if (frame < track.sampledFrame) {
    ...
    while (track.cursor < track.keys.size() &&
           static_cast<float>(track.keys[track.cursor]->frame) < frame)
      ++track.cursor;
  }
...
LocalPose
sampleBone(const std::vector<const VmdBoneKey *> &keys, float frame,
           InterpolationMode fallbackMode = InterpolationMode::bezier) {
  if (keys.empty())
    return {};
  const auto next = std::lower_bound(
      keys.begin(), keys.end(), frame, [](const VmdBoneKey *key, float value) {
        return static_cast<float>(key->frame) < value;
      });
  if (next == keys.begin())
  ...
  const auto previousIndex =
      static_cast<std::size_t>((next - keys.begin()) - 1);
  const auto nextIndex = static_cast<std::size_t>(next - keys.begin());
  const auto *previousControl =
      keys[previousIndex == 0 ? 0 : previousIndex - 1];
  const auto *nextControl = keys[std::min(nextIndex + 1, keys.size() - 1)];
  const float span = static_cast<float>(after->frame - before->frame);
  const float linear =
      span > 0.0F ? (frame - static_cast<float>(before->frame)) / span : 0.0F;
  const bool hasMethods = std::ranges::any_of(keys, [](const auto *key) {
    return std::ranges::any_of(key->methods,
                               [](auto value) { return value != 0; });
  });
  const auto catmull = [&](std::size_t channel) {
    return hasMethods ? after->methods[channel] == 1
                      : fallbackMode == InterpolationMode::catmullRom;
  };
  LocalPose result;
  for (std::size_t axis = 0; axis < 3; ++axis) {
    const auto offset = axis;
    const float t = bezier(
        linear, after->interpolation[offset], after->interpolation[offset + 4],
        after->interpolation[offset + 8], after->interpolation[offset + 12]);
    result.translation[axis] =
        catmull(axis)
            ? catmullRom(previousControl->translation[axis],
                         before->translation[axis], after->translation[axis],
                         nextControl->translation[axis], linear)
            : before->translation[axis] +
                  (after->translation[axis] - before->translation[axis]) * t;
  }
  const float rt =
      bezier(linear, after->interpolation[3], after->interpolation[7],
  ...
  if (catmull(3)) {
    ...
    Quat interpolated{};
    for (std::size_t i = 0; i < 4; ++i) {
      interpolated[i] = catmullRom(control0[i], control1[i], control2[i],
                                   control3[i], linear);
    }
...
LocalPose sampleBone(
    BoneTrack &track, const std::unordered_map<std::uint32_t, BezierLut> &luts,
  ...
  for (std::size_t axis = 0; axis < 3; ++axis) {
    const float t =
        sampleBezier(luts.at(track.curves[curveIndex][axis]), linear);
    result.translation[axis] =
        catmull(axis)
            ? catmullRom(previousControl->translation[axis],
                         before->translation[axis], after->translation[axis],
  ...
  if (catmull(3)) {
    ...
    Quat interpolated{};
    for (std::size_t i = 0; i < 4; ++i) {
      interpolated[i] = catmullRom(controls[0][i], controls[1][i],
                                   controls[2][i], controls[3][i], linear);
    }
    result.rotation = normalize(interpolated);
  } else {
    result.rotation = slerp(before->rotation, after->rotation, rt);
...
void calculateGlobals(const PmxModel &model,
  ...
  const auto resolve = [&](const auto &self, std::size_t index) -> void {
    ...
    if (state[index] == 1) { // malformed parent cycle
      global[index] = {
          add(model.bones[index].position, local[index].translation),
    ...
    if (parent >= 0 && static_cast<std::size_t>(parent) < model.bones.size()) {
      ...
      const auto bindOffset =
          sub(model.bones[index].position,
              model.bones[static_cast<std::size_t>(parent)].position);
      global[index].position =
          add(global[static_cast<std::size_t>(parent)].position,
              rotate(global[static_cast<std::size_t>(parent)].rotation,
                     add(bindOffset, local[index].translation)));
      global[index].rotation =
          multiply(global[static_cast<std::size_t>(parent)].rotation,
                   local[index].rotation);
    } else {
      global[index].position =
          add(model.bones[index].position, local[index].translation);
...
void calculateGlobalSubtree(
    const PmxModel &model, const std::vector<LocalPose> &local,
    std::vector<GlobalPose> &global,
  ...
  const auto update = [&](const auto &self, std::size_t index) -> void {
    ...
    if (parent >= 0 && static_cast<std::size_t>(parent) < global.size()) {
      ...
      const auto bindOffset =
          sub(model.bones[index].position, model.bones[parentIndex].position);
      global[index].position =
          add(global[parentIndex].position,
              rotate(global[parentIndex].rotation,
                     add(bindOffset, local[index].translation)));
      global[index].rotation =
          multiply(global[parentIndex].rotation, local[index].rotation);
    } else {
      global[index].position =
          add(model.bones[index].position, local[index].translation);
      global[index].rotation = local[index].rotation;
...
BoneOrders makeBoneOrders(const PmxModel &model) {
  ...
  for (std::size_t index = 0; index < model.bones.size(); ++index) {
    ...
    (afterPhysics ? result.afterPhysics : result.beforePhysics)
        .push_back(index);
  }
  const auto sortOrder = [&model](BoneOrder &order) {
    std::stable_sort(order.begin(), order.end(),
                     [&model](std::size_t left, std::size_t right) {
...
Float3 quaternionToEuler(Quat rotation) {
  ...
  const float roll = std::atan2(
      2.0F * (rotation[3] * rotation[0] + rotation[1] * rotation[2]),
      1.0F - 2.0F * (rotation[0] * rotation[0] + rotation[1] * rotation[1]));
  const float pitchArgument =
      std::clamp(2.0F * (rotation[3] * rotation[1] - rotation[2] * rotation[0]),
                 -1.0F, 1.0F);
  const float pitch = std::asin(pitchArgument);
  const float yaw = std::atan2(
      2.0F * (rotation[3] * rotation[2] + rotation[0] * rotation[1]),
...
void rebuildBonePoses(const PmxModel &model, const BoneOrder &order,
  ...
  for (const auto index : order) {
    ...
    if (bone.inheritParent >= 0 &&
        static_cast<std::size_t>(bone.inheritParent) < poses.size()) {
      ...
      if ((bone.flags & 0x0080U) != 0) {
        ...
        appendSource.rotation =
            multiply(appendSource.rotation, poses[parent].ikRotation);
      } else {
      ...
      if ((bone.flags & 0x0200U) != 0) {
        pose.append.translation =
            mul(appendSource.translation, bone.inheritRatio);
        pose.local.translation =
            add(pose.local.translation, pose.append.translation);
      }
      if ((bone.flags & 0x0100U) != 0) {
        pose.append.rotation =
            slerp(identity, appendSource.rotation, bone.inheritRatio);
        pose.local.rotation =
            multiply(pose.local.rotation, pose.append.rotation);
      }
    }
...
const VmdIkKey *ikKeyAt(const VmdMotion *motion, float frame) {
  ...
  if (sorted) {
    const auto key =
        std::upper_bound(motion->ik.begin(), motion->ik.end(), frame,
                         [](float value, const VmdIkKey &item) {
                           return value < static_cast<float>(item.frame);
                         });
  ...
  for (const auto &key : motion->ik) {
    if (static_cast<float>(key.frame) <= frame &&
        (result == nullptr || key.frame >= result->frame))
...
void solveIk(const PmxModel &model, const BoneOrder &order,
             std::vector<BoneRuntimePose> &poses, const VmdMotion *motion,
             float frame, std::vector<LocalPose> &localScratch,
             std::vector<GlobalPose> &globalScratch,
             std::vector<std::uint8_t> &globalState) {
  for (const auto ikIndex : order) {
    ...
    for (int loop = 0; loop < loops; ++loop) {
      for (const auto &link : ik.ikLinks) {
        if (link.bone < 0 ||
            static_cast<std::size_t>(link.bone) >= poses.size())
          continue;
        const auto linkIndex = static_cast<std::size_t>(link.bone);
        const auto effector = static_cast<std::size_t>(ik.ikTarget);
        const auto toEffector = normalized(sub(
            poses[effector].global.position, poses[linkIndex].global.position));
        const auto toGoal = normalized(sub(poses[ikIndex].global.position,
                                           poses[linkIndex].global.position));
        ...
        if (length(axis) < 1e-6F && cosine < 0.0F) {
          const Float3 basis = std::abs(toEffector[0]) < 0.9F ? Float3{1, 0, 0}
                                                              : Float3{0, 1, 0};
        ...
        const auto parentRotation =
            parent >= 0 && static_cast<std::size_t>(parent) < poses.size()
                ? poses[static_cast<std::size_t>(parent)].global.rotation
                : Quat{0.0F, 0.0F, 0.0F, 1.0F};
        const auto localDelta = multiply(
            multiply(conjugate(parentRotation), worldDelta), parentRotation);
        poses[linkIndex].ikRotation =
            multiply(localDelta, poses[linkIndex].ikRotation);
        ...
        poses[linkIndex].local.rotation = multiply(
            poses[linkIndex].ikRotation, poses[linkIndex].withoutIk.rotation);
        poses[linkIndex].local.rotation =
            applyIkLimit(poses[linkIndex].local.rotation, link);
        poses[linkIndex].ikRotation =
            multiply(poses[linkIndex].local.rotation,
        ...
        rebuildBonePoses(model, order, poses, localScratch, globalScratch,
                         globalState);
      }
      if (length(
              sub(poses[static_cast<std::size_t>(ik.ikTarget)].global.position,
...
void logLimbDiagnostics(
    const PmxModel &model,
    const std::unordered_map<std::string_view, BoneTrack> &boneTracks,
    const std::vector<LocalPose> &local, const std::vector<GlobalPose> &global,
    float frame) {
  ...
  for (const auto name : names) {
    ...
    const auto sampled = motion == boneTracks.end()
                             ? LocalPose{}
                             : sampleBone(motion->second.keys, frame);
    const auto &pose = local[index];
    const auto &world = global[index];
    log::debug(
        "Bone sample: model=", model.metadata.modelName, ", frame=", frame,
        ", bone=", found->name, ", vmdRot=(", sampled.rotation[0], ",",
        sampled.rotation[1], ",", sampled.rotation[2], ",", sampled.rotation[3],
        ")", ", localRot=(", pose.rotation[0], ",", pose.rotation[1], ",",
        pose.rotation[2], ",", pose.rotation[3], ")", ", globalPos=(",
        world.position[0], ",", world.position[1], ",", world.position[2], ")");
  }
}

Float3 transformPoint(const GlobalPose &pose, const Float3 &bindPosition,
                      const Float3 &value) {
  return add(rotate(pose.rotation, sub(value, bindPosition)), pose.position);
}

void skinSdef(PmxVertex &vertex, const PmxModel &model,
              const std::vector<GlobalPose> &global) {
  const auto first = vertex.bones[0];
  const auto second = vertex.bones[1];
  if (first < 0 || second < 0 ||
      static_cast<std::size_t>(first) >= global.size() ||
  ...
  const auto rotation =
      slerp(global[static_cast<std::size_t>(second)].rotation,
            global[static_cast<std::size_t>(first)].rotation, weight);
  const auto translated0 = transformPoint(
      global[static_cast<std::size_t>(first)],
      model.bones[static_cast<std::size_t>(first)].position, cr0);
  const auto translated1 = transformPoint(
      global[static_cast<std::size_t>(second)],
      model.bones[static_cast<std::size_t>(second)].position, cr1);
  vertex.position =
      add(rotate(rotation, sub(vertex.position, vertex.sdefC)),
          add(mul(translated0, weight), mul(translated1, 1.0F - weight)));
  vertex.normal = normalized(rotate(rotation, vertex.normal));
}

void skinQdef(PmxVertex &vertex, const PmxModel &model,
              const std::vector<GlobalPose> &global) {
  Quat real{};
  Quat dual{};
  Quat pivot{};
  ...
  for (std::size_t influence = 0; influence < 4; ++influence) {
    ...
    if (bone < 0 || static_cast<std::size_t>(bone) >= global.size() ||
        weight == 0.0F)
      continue;
    ...
    auto dualPart = multiplyRaw(
        {translation[0], translation[1], translation[2], 0.0F}, rotation);
...
MmdAnimator::MmdAnimator(const PmxModel &model)
...
void MmdAnimator::setMotion(const VmdMotion *motion) {
  ...
  log::info("Motion compatibility: PMX bones=", compatibility.pmxBoneCount,
            ...
            ", unmatched tracks=",
            compatibility.vmdBoneTrackCount -
                compatibility.matchedBoneTrackCount);
  if (compatibility.vmdBoneTrackCount > 0 &&
      compatibility.matchedBoneTrackCount == 0) {
...
AnimatedModelFrame MmdAnimator::evaluate(float frame, float deltaSeconds,
  ...
  for (std::size_t i = 0; i < model_.bones.size(); ++i) {
    if (const auto found = impl_->boneTracks.find(model_.bones[i].name);
        found != impl_->boneTracks.end()) {
      local[i] = sampleBone(found->second, impl_->bezierLuts, frame,
                            motion_ == nullptr ? InterpolationMode::bezier
                                               : motion_->interpolation);
    }
  }
  if (pose_ != nullptr) {
    for (const auto &value : pose_->bones) {
      const auto bone = std::find_if(
          model_.bones.begin(), model_.bones.end(),
          [&](const PmxBone &item) { return item.name == value.name; });
      if (bone != model_.bones.end()) {
        const auto index =
            static_cast<std::size_t>(bone - model_.bones.begin());
        local[index].translation =
            add(local[index].translation, value.translation);
  ...
  for (std::size_t i = 0; i < model_.morphs.size(); ++i) {
    if (const auto found = impl_->morphTracks.find(model_.morphs[i].name);
        found != impl_->morphTracks.end()) {
      morphWeights[i] = sampleMorph(found->second, frame);
    }
  }
  std::vector<std::uint8_t> morphStack(model_.morphs.size());
  std::function<void(std::size_t, float)> applyMorph = [&](std::size_t index,
                                                           float weight) {
    if (index >= model_.morphs.size() || morphStack[index] != 0 ||
    ...
    for (const auto &offset : morph.offsets) {
      if (morph.type == 0 || morph.type == 9)
        applyMorph(static_cast<std::size_t>(offset.index),
                   weight * offset.scalar);
      ...
      } else if (morph.type == 2 && offset.index >= 0 &&
                 static_cast<std::size_t>(offset.index) < local.size()) {
        const auto bone = static_cast<std::size_t>(offset.index);
        local[bone].translation =
            add(local[bone].translation, mul(offset.vector3, weight));
        local[bone].rotation =
            multiply(local[bone].rotation,
                     slerp({0.0F, 0.0F, 0.0F, 1.0F}, offset.vector4, weight));
      } else if (morph.type == 8) {
        const auto first = offset.index < 0
                               ? std::size_t{0}
                               : static_cast<std::size_t>(offset.index);
        const auto last = offset.index < 0
                              ? result.materials.size()
                              : std::min(first + 1, result.materials.size());
        for (std::size_t material = first; material < last; ++material) {
          ...
          const auto apply4 = [&](Float4 &value, const Float4 &morphValue) {
            for (std::size_t component = 0; component < 4; ++component) {
              if (offset.operation == 0)
                value[component] *=
                    1.0F + (morphValue[component] - 1.0F) * weight;
              else
          ...
          if (offset.operation == 0) {
            destination.shininess *=
                1.0F + (offset.materialVectors[1][3] - 1.0F) * weight;
            destination.edgeSize *=
                1.0F + (offset.materialVectors[2][3] - 1.0F) * weight;
            ...
            apply4(destination.toonAdd, offset.materialVectors[6]);
                 ...
                 static_cast<std::size_t>(offset.index) <
                     model_.rigidBodies.size()) {
        const auto body = static_cast<std::size_t>(offset.index);
        if (length(offset.vector3) <= 1e-8F &&
            length(offset.tertiaryVector3) <= 1e-8F) {
          ...
          impulseLinearLocal[body] =
              add(impulseLinearLocal[body], mul(offset.vector3, weight));
          impulseAngularLocal[body] = add(impulseAngularLocal[body],
                                          mul(offset.tertiaryVector3, weight));
        } else {
          impulseLinear[body] =
              add(impulseLinear[body], mul(offset.vector3, weight));
          impulseAngular[body] =
              add(impulseAngular[body], mul(offset.tertiaryVector3, weight));
        }
      }
    }
    morphStack[index] = 0;
  ...
  rebuildBonePoses(model_, boneOrders.beforePhysics, poses, impl_->localScratch,
                   impl_->globalScratch, impl_->globalState);
  solveIk(model_, boneOrders.beforePhysics, poses, motion_, frame,
          impl_->localScratch, impl_->globalScratch, impl_->globalState);
  for (std::size_t i = 0; i < local.size(); ++i)
  ...
  if (physics_ != nullptr && physics_->available()) {
        ...
        (previousFrame_ >= 0.0F &&
         (frameDelta < 0.0F ||
          (std::abs(frameDelta) > 1e-6F &&
           (deltaSeconds <= 0.0F ||
            std::abs(frameDelta - expectedFrameDelta) > 2.0F))));
    ...
    for (std::size_t bodyIndex = 0; bodyIndex < model_.rigidBodies.size();
         ++bodyIndex) {
      ...
      value.position =
          add(global[bone].position, rotate(global[bone].rotation, offset));
    ...
    for (std::size_t body = 0; body < model_.rigidBodies.size(); ++body) {
      ...
      if (length(impulseLinear[body]) > 0.0F ||
          length(impulseAngular[body]) > 0.0F) {
        physics_->applyImpulse(body, mul(impulseLinear[body], impulseScale),
                               mul(impulseAngular[body], impulseScale), false);
      }
      if (length(impulseLinearLocal[body]) > 0.0F ||
          length(impulseAngularLocal[body]) > 0.0F) {
        physics_->applyImpulse(
            body, mul(impulseLinearLocal[body], impulseScale),
    ...
    for (std::size_t bodyIndex = 0; bodyIndex < model_.rigidBodies.size();
      ...
      if (mode == 0 || body.bone < 0 ||
          static_cast<std::size_t>(body.bone) >= global.size())
        continue;
      const auto bone = static_cast<std::size_t>(body.bone);
      physicsBones[bone] = true;
      const auto bodyPose = physics_->bodyTransform(bodyIndex);
      const auto offset = sub(body.position, model_.bones[bone].position);
      const auto boneRotation =
          multiply(bodyPose.rotation, conjugate(eulerRotation(body.rotation)));
      ...
      if (mode == 2)
        ...
        targetBonePosition =
            sub(bodyPose.position, rotate(boneRotation, offset));
      const auto parent = model_.bones[bone].parent;
      if (parent >= 0 && static_cast<std::size_t>(parent) < global.size()) {
        const auto parentIndex = static_cast<std::size_t>(parent);
        local[bone].rotation =
            multiply(conjugate(global[parentIndex].rotation), boneRotation);
        const auto bindOffset = sub(model_.bones[bone].position,
                                    model_.bones[parentIndex].position);
        local[bone].translation =
            sub(rotate(conjugate(global[parentIndex].rotation),
                       sub(targetBonePosition, global[parentIndex].position)),
                bindOffset);
      } else {
        local[bone].rotation = boneRotation;
        local[bone].translation =
            sub(targetBonePosition, model_.bones[bone].position);
      }
  ...
  rebuildBonePoses(model_, boneOrders.afterPhysics, poses, impl_->localScratch,
                   impl_->globalScratch, impl_->globalState);
  solveIk(model_, boneOrders.afterPhysics, poses, motion_, frame,
          impl_->localScratch, impl_->globalScratch, impl_->globalState);
  for (std::size_t i = 0; i < local.size(); ++i) {
    local[i] = poses[i].local;
    global[i] = poses[i].global;
  }
  if (previousFrame_ < 0.0F ||
      static_cast<int>(previousFrame_) != static_cast<int>(frame)) {
  ...
  for (auto &vertex : result.vertices) {
    ...
    const std::size_t influenceCount =
        vertex.weightType == PmxWeightType::bdef1
            ? 1
            : (vertex.weightType == PmxWeightType::bdef2 ||
                       vertex.weightType == PmxWeightType::sdef
                   ? 2
                   : 4);
    for (std::size_t influence = 0; influence < influenceCount; ++influence) {
      ...
      position =
          add(position,
              mul(transformPoint(
                      global[static_cast<std::size_t>(bone)],
                      model_.bones[static_cast<std::size_t>(bone)].position,
                      vertex.position),
                  weight));
      normal = add(normal,
                   mul(rotate(global[static_cast<std::size_t>(bone)].rotation,
                              vertex.normal),
                       weight));
      totalWeight += weight;
    }
...
PreviewNormalization previewNormalization(const PmxModel &model) {
  ...
  result.center = {(minimum[0] + maximum[0]) * 0.5F,
                   (minimum[1] + maximum[1]) * 0.5F,
                   (minimum[2] + maximum[2]) * 0.5F};
  result.scale =
      1.8F / std::max({maximum[0] - minimum[0], maximum[1] - minimum[1],
                       maximum[2] - minimum[2], 0.001F});
  return result;
}

void normalizeForPreview(std::vector<PmxVertex> &vertices,
                         const PreviewNormalization &normalization) {
  for (auto &vertex : vertices)
    for (std::size_t axis = 0; axis < 3; ++axis) {
      vertex.position[axis] =
          (vertex.position[axis] - normalization.center[axis]) *
          normalization.scale;
    }
}
