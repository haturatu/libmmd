...
PmxIndexWidths pmx::chooseIndexWidths(const PmxModel& model, PmxSaveOptions options) noexcept {
    const PmxIndexWidths required{
        requiredVertexIndexWidth(model.vertices.size()),
        requiredSignedIndexWidth(model.textures.size()),
        requiredSignedIndexWidth(model.materials.size()),
        requiredSignedIndexWidth(model.bones.size()),
        requiredSignedIndexWidth(model.morphs.size()),
        requiredSignedIndexWidth(model.rigidBodies.size()),
    };
...
PmxSaveReport pmx::save(const std::filesystem::path& path, const PmxModel& model, PmxSaveOptions options) {
    ...
    const std::array<std::uint8_t, 8> settings{encoding, model.metadata.additionalUvCount,
                                                widths.vertex, widths.texture, widths.material,
                                                widths.bone, widths.morph, widths.rigidBody};
    ...
    for (const auto index : model.indices) writeVertexIndex(output, index, widths.vertex, "vertex index");
    ...
    for (const auto& material : model.materials) {
        ...
        writeIndex(output, material.textureIndex, widths.texture, "texture index"); writeIndex(output, material.sphereTextureIndex, widths.texture, "sphere texture index"); write(output, material.sphereMode, "sphere mode"); write(output, material.toonMode, "toon mode");
        if (material.toonMode == 0) writeIndex(output, material.toonTextureIndex, widths.texture, "toon texture index"); else write(output, static_cast<std::uint8_t>(material.toonTextureIndex), "shared toon index");
    ...
    for (const auto& bone : model.bones) {
        writeText(output, bone.name, "bone name"); writeText(output, bone.englishName, "bone English name"); writeArray(output, bone.position, "bone position"); writeIndex(output, bone.parent, widths.bone, "bone parent"); write(output, bone.deformLayer, "bone layer"); write(output, bone.flags, "bone flags");
        if ((bone.flags & 0x0001U) != 0) writeIndex(output, bone.tailBone, widths.bone, "bone tail"); else writeArray(output, bone.tailOffset, "bone tail offset");
        if ((bone.flags & 0x0300U) != 0) { writeIndex(output, bone.inheritParent, widths.bone, "bone inherit parent"); write(output, bone.inheritRatio, "bone inherit ratio"); }
